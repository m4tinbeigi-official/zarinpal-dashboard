    ...
    <aside className="hidden lg:flex flex-col fixed right-0 top-0 h-full w-64 bg-zp-dark text-white z-40">
      ...
      <div className="px-6 py-5 border-b border-white/10">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-zp-yellow rounded-lg flex items-center justify-center text-zp-dark font-black text-sm">Z</div>
          <div>
            <p className="font-bold text-sm">زرین‌پال</p>
            <p className="text-[10px] text-white/50">داشبورد تحلیلی</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-3 border-b border-white/10">
        <label className="text-[11px] text-white/50 mb-1 block">پذیرنده</label>
        <select
          value={selectedMerchant}
          onChange={(e) => onMerchantChange(e.target.value)}
          className="w-full bg-white/10 text-white text-sm rounded-lg px-3 py-2 border border-white/10 focus:outline-none focus:border-zp-yellow"
        >
          {merchants.map((m) => (
        ...
        {tabs.map((t) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition ${
              activeTab === t.id
                ? "bg-zp-yellow text-zp-dark font-bold"
      ...
      <div className="px-4 py-3 border-t border-white/10">
        <button
          ...
          className="w-full text-xs text-white/50 hover:text-zp-yellow transition py-2"
        >
